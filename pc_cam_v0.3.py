...
def open_video_stream(source, width=None, height=None):
    """
    Opens a video stream using FFmpeg with TCP and Zero-Latency settings.
    """
    # 1. Force TCP to prevent packet loss (gray screens)
    # 2. 'nobuffer' and 'flush_packets' reduce the 10s delay to near zero
    os.environ["OPENCV_FFMPEG_CAPTURE_OPTIONS"] = "rtsp_transport;tcp|fflags;nobuffer|flags;low_delay"
    
    print(f"Opening stream with FFmpeg/TCP (Low Latency): {source}")
    ...
    # Reduce internal buffer size to 1 frame (default is usually 3-5)
...
#                     action='store_true')
# parser.add_argument('--ocr-thresh', help='Minimum OCR confidence threshold (example: "0.5")', 
#                     default=0.94, type=float)
...
# img_source = "rtsp://admin:FazNiz!12@192.168.1.64:554/Streaming/Channels/101/"
img_source = "rtsp://admin:FazNiz!12@192.168.1.64:554/Streaming/Channels/102/"
min_thresh = 0.7
user_res = args.resolution
# record = args.record
# ocr_thresh = args.ocr_thresh
        ...
        if conf > min_thresh:
            ...
            try:
                ...
                if plate_crop.size > 0:
                    # ocr_results = reader.readtext(plate_crop, detail=0, min_score=0.5, allowlist='ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-')
                    # ocr_text = "".join(ocr_results).strip()# Change detail=0 to detail=1 to get (bbox, text, prob)
